...
async function loadData(){
  ...
  const promises = [qRides, qBookings, qVehicles, qMyRides];
  if (isAdmin()) {
  ...
  if (isAdmin()) { state.admin = res[4].data || {}; state.reports = res[5].data || []; state.users = res[6].data || []; }
}
...
function render(){
  if (!state.session) return renderAuth();
  const tabs = navTabs();
  app.innerHTML = `<div class="shell"><div class="top"><div class="brand"><div class="row"><div class="logo">RM</div><div><div class="title">${cfg.appName}</div><div class="sub">${esc(state.profile?.full_name)} · ${esc(role())}</div></div></div><button class="avatar" id="logoutBtn">Logout</button></div></div><main class="content">${view()}</main>${state.modal || ''}<nav class="tabs">${tabs.map(t=>`<button class="tab ${state.tab===t.id?'active':''}" data-tab="${t.id}">${t.icon}<br>${t.label}</button>`).join('')}</nav></div>`;
...
function navTabs(){
  ...
  if (role()==='driver') return [{id:'home',label:'Home',icon:'🏠'},{id:'create',label:'Post',icon:'➕'},{id:'requests',label:'Requests',icon:'📩'},{id:'safety',label:'Safety',icon:'🛟'},{id:'profile',label:'Profile',icon:'⚙️'}];
  return [{id:'home',label:'Search',icon:'🔎'},{id:'bookings',label:'Bookings',icon:'🎫'},{id:'safety',label:'Safety',icon:'🛟'},{id:'profile',label:'Profile',icon:'⚙️'}];
}
...
function passengerHome(){
  ...
  const cards = rides.map(rideCard).join('') || `<div class="empty">No matching rides yet.</div>`;
  return `<div class="card"><div class="h1">Find a safe seat</div><div class="grid"><div class="grid2"><label>From<input id="fFrom" value="${esc(state.filters.from)}" placeholder="Wah Cantt"></label><label>To<input id="fTo" value="${esc(state.filters.to)}" placeholder="Islamabad"></label></div><label>Filter<select id="fRule"><option value="safe" ${state.filters.ride_rule==='safe'?'selected':''}>Only safe matching rides</option><option value="all" ${state.filters.ride_rule==='all'?'selected':''}>All rides</option><option value="female_only" ${state.filters.ride_rule==='female_only'?'selected':''}>Female-only seats</option><option value="male_only" ${state.filters.ride_rule==='male_only'?'selected':''}>Male-only seats</option><option value="family_only" ${state.filters.ride_rule==='family_only'?'selected':''}>Family only</option></select></label></div></div>${cards}`;
}

function rideCard(r){
  const c = compatible(r); const existing = state.myBookings.find(b => b.ride_id === r.id && ['pending','accepted'].includes(b.status));
  return `<div class="card ride"><div class="row"><div><div class="route">${esc(r.from_city)} → ${esc(r.to_city)}</div><div class="small muted">${esc(r.pickup_area||'Pickup')} to ${esc(r.dropoff_area||'Dropoff')}</div></div><div class="fare">${money(r.price_per_seat)}</div></div><div class="meta"><span class="pill ok">${r.seats_left} seats left</span><span class="pill">${esc(ruleLabel(r.ride_rule))}</span><span class="pill ${c.ok?'ok':'bad'}">${esc(existing ? existing.status : c.msg)}</span></div><div class="small muted">${fmt(r.departure_at)} · ${esc(r.driver_name)} · ${esc(r.driver_gender)} driver · ${esc(r.car_model||'Car')} ${esc(r.plate_number||'')}</div><div class="grid2"><button class="btn ghost" data-details="${r.id}">Details</button><button class="btn green" data-book="${r.id}" ${(!c.ok || existing)?'disabled':''}>${existing?'Requested':'Book seat'}</button></div></div>`;
}
...
function driverHome(){
  const rides = state.myRides.map(r => `<div class="card"><div class="row"><div><div class="route">${esc(r.from_city)} → ${esc(r.to_city)}</div><div class="small muted">${fmt(r.departure_at)} · ${esc(ruleLabel(r.ride_rule))}</div></div><span class="pill ${r.status==='open'?'ok':'bad'}">${r.status}</span></div><div class="meta"><span class="pill ok">${r.seats_left} left / ${r.total_seats}</span><span class="pill">${money(r.price_per_seat)}</span></div><div class="grid2"><button class="btn ghost" data-details="${r.id}">Details</button><button class="btn danger" data-close-ride="${r.id}">Close ride</button></div></div>`).join('') || `<div class="empty">No rides posted yet.</div>`;
...
function createRideView(){
  ...
  return `<div class="card"><div class="h1">Post a ride</div>${!state.vehicles.length?'<div class="alert">Add your car first in profile. Ride post needs a verified vehicle record.</div>':''}<form id="rideForm" class="grid"><label>Vehicle<select name="vehicle_id" required>${vehicleOptions}</select></label><div class="grid2"><label>From city<input name="from_city" required placeholder="Wah Cantt"></label><label>To city<input name="to_city" required placeholder="Islamabad"></label></div><div class="grid2"><label>Pickup area<input name="pickup_area" required></label><label>Dropoff area<input name="dropoff_area" required></label></div><label>Date & time<input name="departure_at" type="datetime-local" required></label><div class="grid2"><label>Total seats<input name="total_seats" type="number" min="1" max="6" value="3" required></label><label>Price/seat<input name="price_per_seat" type="number" min="0" value="500" required></label></div><label>Passenger rule<select name="ride_rule"><option value="mixed">Mixed</option><option value="male_only">Male passengers only</option><option value="female_only">Female passengers only</option><option value="family_only">Family only</option></select></label><label>Notes<textarea name="notes" placeholder="Luggage, AC, exact gate, etc."></textarea></label><button class="btn green" ${!state.vehicles.length?'disabled':''}>Post ride</button></form></div>`;
}
